<x-app-layout title="Ubah Kasir">

    <div class="bg-lB w-1/2 p-3 border-2 border-black rounded-md shadow-md">
        <div class="mb-5 flex justify-between relative">
            <a href="{{ route('admin.daftar-cashier') }}" class="z-20">
                <i class="fas fa-arrow-left text-4xl"></i>
            </a>
            <div class="absolute inset-0 flex items-center justify-center">
                <h1 class="font-poppins text-4xl font-bold">Ubah Kasir</h1>
            </div>
        </div>

        @foreach ( $cashier as $data )
        <form action="{{ route('admin.update-cashier', [$data->cashier_id]) }}" method="POST" class="font-roboto">
            @csrf
            @method('put')
